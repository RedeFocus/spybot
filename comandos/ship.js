...
if(!user2) return message.reply("você deve mencionar alguém!\n\n Exemplo:\n m-ship @PedroMzika @Masuno ")
...
var mensagem = porcentagem <= 5 ? `${porcentagem}% ${aido[i]}` : porcentagem <= 10 ? `${porcentagem}% [█████-----] Um dia talvez! 🤔` : porcentagem <= 50 ? `${porcentagem}% [███████-----] Eles são perfeitos! 😍 ` : porcentagem <= 70 ? `${porcentagem}% [███████----] Já deveriam estar casados! 💍 ` : porcentagem <= 100 ? `${porcentagem}% [██████████] Casal perfeito, ninguém os separa! 💍` : `Casal perfeito, Ninguem os separa! 💍`
message.channel.send({
    embed: {
    "fields": [
      {
        "name": `Será que nós temos um novo casal aqui ?`,
